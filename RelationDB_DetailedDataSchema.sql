...
ALTER TABLE microclimate
    ADD CONSTRAINT fk_humidity_id FOREIGN KEY (humidity_id) 
    REFERENCES humidity(id) ON DELETE CASCADE ON UPDATE CASCADE;

-- Add foreign key constraints to planpattern
ALTER TABLE planpattern
    ADD CONSTRAINT fk_optimalmicroclimate_id FOREIGN KEY (optimalmicroclimate_id) 
    REFERENCES microclimate(id) ON DELETE CASCADE ON UPDATE CASCADE;

ALTER TABLE planpattern
    ADD CONSTRAINT fk_planparameters_id FOREIGN KEY (planparameters_id) 
    REFERENCES planparameters(id) ON DELETE CASCADE ON UPDATE CASCADE;

-- Add foreign key constraints to microclimateplan
ALTER TABLE microclimateplan
    ADD CONSTRAINT fk_planpattern_id FOREIGN KEY (planpattern_id) 
    REFERENCES planpattern(id) ON DELETE CASCADE ON UPDATE CASCADE;

ALTER TABLE microclimateplan
    ADD CONSTRAINT fk_initiallymicroclimate_id FOREIGN KEY (initiallymicroclimate_id) 
    REFERENCES microclimate(id) ON DELETE CASCADE ON UPDATE CASCADE;
...
ALTER TABLE microclimateplan
    ADD CONSTRAINT fk_topic_id FOREIGN KEY (topic_id) 
